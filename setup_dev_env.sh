...
MAIN_APP_SERVICE="adaptercms"
# Path to the wait-for-it.sh script (used inside the main app container)
# This script is used to wait for the database service to be ready before starting the main application.
# It is downloaded from a remote URL and saved in the current directory.
# The script is then copied into the main application container.
...
show_help() {
    cat << EOF
${BOLD}Adapter CMS Development Environment Setup${RESET}
...
print_info "You can access Adapter CMS at: ${BOLD}http://localhost:8080${RESET}"
