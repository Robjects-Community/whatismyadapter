...
// Register the upload.file type from josegonzalez/cakephp-upload plugin
// This is needed for fixtures that use upload fields
use Cake\Database\TypeFactory;
if (class_exists('Josegonzalez\Upload\Database\Type\FileType')) {
    TypeFactory::map('upload.file', 'Josegonzalez\Upload\Database\Type\FileType');
}
...
// Create schema for test database from schema files
$schemaDir = $root . '/tests/schema';
if (is_dir($schemaDir)) {
    $connection = ConnectionManager::get('test');
    
    foreach (glob($schemaDir . '/*.php') as $schemaFile) {
        $tableName = basename($schemaFile, '.php');
        $def = include $schemaFile;
        
        if (is_callable($def)) {
            // Closure-based schema: create table from TableSchema
            $schema = new \Cake\Database\Schema\TableSchema($tableName);
            $schema = $def($schema); // Call the closure to configure the schema
            
            // Convert schema to SQL and execute to create the table
            $sql = $schema->createSql($connection);
            foreach ($sql as $statement) {
                try {
                    $connection->execute($statement);
                } catch (\Exception $e) {
                    // Silently log schema warnings - most are harmless SQLite quirks
                    if (str_contains($e->getMessage(), 'already exists') === false) {
                        fwrite(STDERR, "Schema warning for {$tableName}: " . $e->getMessage() . "\n");
                    }
                }
            }
        }
