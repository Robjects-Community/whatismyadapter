...
class AiProductMatcherService
{
     ...
     */
    private function getAiScore(Product $product, array $answers): ?float
    {
        if (!$this->aiService) {
            return null;
        }
     ...
     */
    private function buildAiScoringPrompt(Product $product, array $answers): string
    {
     ...
     */
    private function buildProductSummary(Product $product): string
    {
        $summary = [];
     ...
     * @return array
     */
    private function getMatchReasons(Product $product, array $answers): array
    {
        $reasons = [];
        ...
        if (!empty($answers['device_type'])) {
            ...
            if (
                stripos($product->device_category ?? '', $deviceType) !== false ||
