...
# Deployment script for WillowCMS with APP_PORT fix
set -euo pipefail

DROPLET_IP="$1"

echo "Deploying WillowCMS to $DROPLET_IP..."

# Create necessary files locally with fixes
...
scp -i ~/.ssh/id_ed25519_digital_ocean_droplet docker-compose.prod.yml.fixed root@$DROPLET_IP:/opt/willowcms/docker-compose.prod.yml

# Upload other necessary files
scp -i ~/.ssh/id_ed25519_digital_ocean_droplet -r app/ root@$DROPLET_IP:/opt/willowcms/
scp -i ~/.ssh/id_ed25519_digital_ocean_droplet -r tools/ root@$DROPLET_IP:/opt/willowcms/
scp -i ~/.ssh/id_ed25519_digital_ocean_droplet -r docker/ root@$DROPLET_IP:/opt/willowcms/
scp -i ~/.ssh/id_ed25519_digital_ocean_droplet Dockerfile root@$DROPLET_IP:/opt/willowcms/
scp -i ~/.ssh/id_ed25519_digital_ocean_droplet .env.example root@$DROPLET_IP:/opt/willowcms/
scp -i ~/.ssh/id_ed25519_digital_ocean_droplet .dockerignore root@$DROPLET_IP:/opt/willowcms/

echo "Creating .env file on droplet..."
ssh -i ~/.ssh/id_ed25519_digital_ocean_droplet root@$DROPLET_IP << 'REMOTE_COMMANDS'
cd /opt/willowcms
...
MYSQL_DATABASE=willowcms_prod
MYSQL_USER=willowcms
MYSQL_PASSWORD=willowcms_password123
...
# Port Configuration
WILLOWCMS_HTTP_PORT=8080
REDIS_COMMANDER_HTTP_PORT=8084
PHPMYADMIN_HTTP_PORT=8082
...
WILLOWCMS_LOGS_VOLUME=willowcms_logs
WILLOWCMS_TMP_VOLUME=willowcms_tmp
ENV_EOF
...
echo "Deployment script completed. Checking final status..."

# Check final container status
ssh -i ~/.ssh/id_ed25519_digital_ocean_droplet root@$DROPLET_IP 'cd /opt/willowcms && docker compose logs willowcms --tail=20'

echo "WillowCMS should be available at: http://$DROPLET_IP:8080"